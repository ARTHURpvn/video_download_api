#!/usr/bin/env python3
"""
YouTube Downloader - Instalador para Windows
Instala dependências e cria executável na Área de Trabalho
"""
...
class InstaladorWindows:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("YouTube Downloader - Instalador Windows")
    ...
    def create_ui(self):
        ...
        tk.Label(main, text="Instalador para Windows", font=('Arial', 11),
        ...
        self.log = scrolledtext.ScrolledText(log_frame, height=15, width=75,
            font=('Consolas', 9), bg="#000", fg="#0f0", wrap=tk.WORD, state='disabled')
    ...
    def install(self):
        try:
            ...
            self.status("⏳ [1/5] Verificando Python...")
            ...
            self.status("⏳ [2/5] Atualizando pip...")
            self.write("\n[2/5] Atualizando pip")
            self.exec_cmd([sys.executable, "-m", "pip", "install", "--upgrade", "pip", "-q"])
            time.sleep(0.3)

            self.status("⏳ [3/5] Instalando dependências...")
            ...
            self.status("⏳ [4/5] Criando diretórios...")
            self.write("\n[4/5] Configurando diretórios")
            dl_dir = os.path.join(os.path.expanduser("~"), "Downloads", "Videos Baixados")
            os.makedirs(dl_dir, exist_ok=True)
            self.write(f"✓ {dl_dir}")
            time.sleep(0.3)

            self.status("⏳ [5/5] Compilando executável...")
            self.write("\n[5/5] Compilando aplicativo (aguarde 5-10 min)")
            build_file = os.path.join(os.path.dirname(__file__), "..", "build_executable.py")
            if self.exec_cmd([sys.executable, build_file]):
                desktop = os.path.join(os.path.expanduser("~"), "Desktop")
                exe_path = os.path.join(desktop, "YouTubeDownloader.exe")
                if os.path.exists(exe_path):
                    self.write(f"\n✓✓✓ EXECUTÁVEL CRIADO! ✓✓✓")
                    self.write(f"📁 Local: {exe_path}")

            self.write("\n" + "═" * 65)
            self.write("  ✅ INSTALAÇÃO CONCLUÍDA!")
            self.write("═" * 65)
            self.write("\nVá até sua Área de Trabalho e abra YouTubeDownloader.exe")
    ...
    def show_done(self):
        if messagebox.askyesno("Concluído!",
            ...
            try:
                exe = os.path.join(os.path.expanduser("~"), "Desktop", "YouTubeDownloader.exe")
